          ...
          while offset < chunk_end:   #True    
           ...
           # print(result)
            ...
            with driver.session(database="socialnetwork1") as session_dest:
                batch_size = 300  # Adjust batch size based on your environment and testing
                        ...
                        # print(project)
                         if project[:2]=='ar': 
                           ...
                           query = ("MATCH (g:GARD) "
                                    ...
                                    "MATCH (n1:Author)--(n2:Article) "
                                    "WHERE  n2.pubmed_id = $id_ "
                                    "merge (n1)-[:Research_on]->(cl)"
                                   )
                           current_batch.append((query, {"id_": id_, "Cluster_ID": Cluster_ID}))
                         if project[:3]=='NCT':
                           id_= project
                           query = ("MATCH (g:GARD) "
                                    f"SKIP {offset} LIMIT {batch_size_q} " 
                                    " match (g)--(cl:Cluster)"
                                    " where cl.Cluster_ID= $Cluster_ID "
                                    "MATCH (n1:Author1)--(n2:ClinicalTrial) "
                                    "WHERE  n2.NCTId = $id_ "
                                    "merge (n1)-[:Research_on]->(cl)"
                                   )
                           current_batch.append((query,  {"id_": id_, "Cluster_ID": Cluster_ID}))
                           ...
                           query = ("MATCH (g:GARD) "
                                    ...
                                    " match (g)--(cl:Cluster)"
                                    " where cl.Cluster_ID= $Cluster_ID "
                                    "MATCH (n1:Author1)--(n2:Grant) "
                                    "WHERE  n2.application_id = $id_ "
                                    "merge (n1)-[:Research_on]->(cl)"
                                   )
                           current_batch.append((query,  {"id_": id_, "Cluster_ID": Cluster_ID}))
...
def run_parallel_queries(start, total_articles, num_processes=80):
...
if __name__ == "__main__":
    ...
    run_parallel_queries(start,total_articles, num_processes=80)
